...
\author{\IEEEauthorblockN{Ali Makkai}
...
Mental health chatbots have attracted growing research attention at the intersection of AI and healthcare systems engineering. Fitzpatrick et al.~\cite{fitzpatrick2017} demonstrated that conversational agents delivering cognitive-behavioral therapy (CBT) content can significantly reduce depression symptoms in college students. However, fully autonomous therapeutic agents raise safety concerns: Bickmore et al.~\cite{bickmore2018} showed that users disclose sensitive information to chatbots at rates comparable to human counselors, creating risks when the system lacks adequate safety guardrails.
...
\begin{figure}[t]
\centering
\fbox{\parbox{0.95\linewidth}{\small\centering
\textbf{System Architecture}\\[4pt]
User Message $\rightarrow$ \textbf{Safety Classifier} (Tier 1--4)\\
$\downarrow$\\
Tier 3/4: Hardcoded response (redirect / crisis)\\
Tier 1/2: \textbf{RAG Retrieval} (ChromaDB, top-3)\\
$\downarrow$\\
\textbf{Qwen3-8B + LoRA Adapter}\\
(tier-specific system prompt + retrieved context)\\
$\downarrow$\\
\textbf{Post-processing} (strip thinking tags)\\
$\downarrow$\\
Response to User
}}
